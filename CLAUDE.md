...
- Errors: wrap with `%w` and context (`fmt.Errorf("registering environment: %w", err)`), never swallow silently, never `panic` outside of `main` wiring/startup validation and truly unrecoverable invariant violations (e.g. `NewRegistry`'s duplicate-provider panic — a wiring bug, not a runtime condition). This includes a bare pass-through of an external call's return (`return pkg.Call()`) — capture the error and wrap it even when the call looks infallible; `wrapcheck` enforces this in CI, so an unwrapped external error fails the build, not just review.
...
- Errors returned to clients are structured (a stable code plus a human message), never a raw Go error string — raw errors leak internal detail and are not stable enough for a CLI/agent to branch on. `controlplane/internal/api/errors.go`'s `errorResponse` type and `writeError`/`writeInternalError`/`writeJSON` helpers are that shared shape — use them rather than formatting a response ad hoc in a new handler.
- Handlers are standalone functions that take their dependencies as explicit arguments and return `http.Handler` (a "maker function" over a store, issuer, etc.), never methods on a shared `*Server`/`*api.Server`-style struct — a method has silent access to every field on the struct whether it uses it or not, which hides what a given handler actually depends on and makes it awkward to test in isolation. `Server.Routes()` is the one place every route and its exact dependencies are listed; add new routes there, don't scatter route registration elsewhere.
- Every `cmd/<binary>/main.go` is a thin entrypoint over an importable package (`controlplane/internal/server.Run`, `cli/internal/cmd.NewRootCmd`), not where the actual logic lives — `main` just wires `os.Getenv`/`os.Args`/`os.Exit` into that package's exported entry point. This is what makes the real logic testable with a fake environment instead of only by actually running the binary, and it's the same shape for every binary in the repo, not something reinvented per command.
- Routing is stdlib `net/http.ServeMux` (Go 1.22+ method+path patterns, e.g. `"POST /v1/agents"`) — no router framework (`chi`, `gin`, etc.) unless a concrete, documented need for route grouping or richer middleware composition actually arrives; stdlib has covered everything so far.
- No OpenAPI spec exists yet, deliberately — see [docs/DELIVERY.md](docs/DELIVERY.md) "Open questions" for why (`swaggo` was tried and reverted: Swagger 2.0 only, unenforced comment annotations) and what the real candidates are (`oapi-codegen`, `huma`) once the surface justifies picking one. Don't reach for `swaggo` again without re-reading that note first.
...
- All SQL goes through `sqlc`, never hand-written scanning code. Each domain package owns its own query files and generated output (`internal/<domain>/queries/*.sql` → `internal/<domain>/sqlcgen`), scoped per-package rather than one shared generated package, so `tenant` can't accidentally reach into `actor`'s queries. `make generate` regenerates; `make generate-check` (`sqlc diff`) fails CI if a query changed without regenerating. A hand-written `Store` type wraps the generated `Queries` — callers use the domain's own types (`tenant.Tenant`, not `sqlcgen.Tenant`), so a `sqlc` regen never changes a caller-facing shape.
- Every `NewStore` constructor takes `sqlcgen.DBTX` (an interface satisfied by both `*sql.DB` and `*sql.Tx`), not a concrete `*sql.DB` — so an operation that must be atomic across several stores (e.g. `POST /v1/setup` creating a tenant, team, actor and local login together) can open one `*sql.Tx` and construct transaction-scoped stores from it, rather than risking a partial write on failure.
- Tests touching the database use a real, ephemeral Postgres via `testcontainers-go` (`controlplane/internal/dbtest.New(t)`), applying the actual migrations — not a mock, and not skipped by default the way the AWS/CI-provider integration tests above are. Requires a reachable Docker daemon; skips gracefully if none is found.
- No credential the control plane needs is ever supplied by the operator as a secret to configure before first boot — see [Decision 018](docs/DECISIONS.md). The admin login is set by the operator through a one-time `POST /v1/setup`, and the JWT signing key is generated once and persisted in Postgres on first startup. Don't reach for an env-var-supplied secret for a new credential without checking whether this pattern already covers it.
...
- `pre-commit` runs the fast Go gates (`go vet`, `golangci-lint`, `gosec`), general hygiene checks, `detect-secrets`, and `commitlint` before a commit lands — same shape as [ssmctl](https://github.com/rhysmcneill/ssmctl)'s config, mirroring the Makefile targets rather than re-encoding the checks separately. Lands in Phase 0. Run `make setup` right after cloning, before your first commit — it installs `golangci-lint`/`goimports`/`gosec` and registers the hooks; code written before `make setup` has run gets none of this enforcement, which is exactly how `pkg/identity`/`pkg/ci` accumulated lint debt before CI existed.
